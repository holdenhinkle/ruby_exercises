VALID_CHOICES = {
  rock: { input: ['r', 'rock'], beats: ['scissors', 'lizard'] },
  paper: { input: ['p', 'paper'], beats: ['rock', 'spock'] },
  scissors: { input: ['s', 'scissors'], beats: ['paper', 'lizard'] },
  spock: { input: ['sp', 'spock'], beats: ['scissors', 'rock'] },
  lizard: { input: ['l', 'lizard'], beats: ['paper', 'spock'] }
}
...
def computer_throws_hand
  VALID_CHOICES.keys.sample.to_s
end

def valid_choice?(user_input)
  VALID_CHOICES.each do |hand, _|
    return true if VALID_CHOICES[hand][:input].include?(user_input)
  end
  false
end
...
def translate_input_to_hand_name(user_input)
  VALID_CHOICES.each do |hand, _|
    return hand.to_s if VALID_CHOICES[hand][:input].include?(user_input)
  end
end
...
def win?(player_hand, other_player_hand)
  VALID_CHOICES[player_hand.to_sym][:beats].include?(other_player_hand)
end
